...
namespace EditorNS
{
    ...
    public class TextStyle : Style
    {
                ...
                if (range.tb.ImeAllowed)
                    for (int i = range.Start.iChar; i < range.End.iChar; i++)
                    {
                        ...
                        if (size.Width > range.tb.CharWidth * 1.5f)
                            gr.DrawString(line[i].c.ToString(), f, ForeBrush, x, y, stringFormat); //y+ range.tb.CharHeight / 4
                        else
                            gr.DrawString(line[i].c.ToString(), f, ForeBrush, x, y, stringFormat);
        ...
        //获取字符串中文数量
        //private int GetLineIsChinese(Line list)
        //{
        //    int isChinese = 0;
    ...
    public class SelectionStyle : Style
    {
        ...
        public SelectionStyle(Brush backgroundBrush, Brush foregroundBrush ) //Brush foregroundBrush = null
        {
            this.BackgroundBrush = backgroundBrush;
        ...
        public override void Draw(Graphics gr, Point position, Range range)
        {
            ...
            if (BackgroundBrush != null)
            {
                ...
                for (int i = range.Start.iChar; i < range.End.iChar; i++)
                {
                    if (i >= range.tb.lines[range.Start.iLine].Count)
                        break;
                    if (range.tb.IsChina(range.tb.lines[range.Start.iLine][i].c))
                        Xwidth += (int)sizef.Width;
                    else
                        Xwidth += range.tb.CharWidth;
                }

                gr.SmoothingMode = SmoothingMode.None;
                var rect = new Rectangle(position.X, position.Y, Xwidth, range.tb.CharHeight); //(range.End.iChar - range.Start.iChar) * range.tb.CharWidth==>Xwidth
