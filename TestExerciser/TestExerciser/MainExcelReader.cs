...
{
    public partial class MainExcelReader : Skin_Mac
    {
        ...
        private void createdScript()
        {
            ...
            for (int i = 0; i < this.ds.Tables[0].Rows.Count; i++)
            {
                ...
                {
                    if (readCell(i + j, 3) != null && readCell(i + j, 3) != "")
                    {
                        string tsFolder = slaMFolder + "\\" + readCell(i + j, 3).ToString().Replace("\n", "");
                        createFolder(tsFolder);
                        createFile(tsFolder + "\\" + readCell(i + j, 3).ToString().Replace("\n", "") + ".ts");
                        testSuiteNO = readCell(i + j, 3).ToString().Replace("\n", "");
                        testSuiteFilePath = tsFolder + "\\" + readCell(i + j, 3).ToString().Replace("\n", "") + ".ts";

                        for (int k = 2; k < this.ds.Tables[0].Rows.Count-(i+2); k++)
                        {
                            if (readCell(i + k, 4) != null && readCell(i + k, 4) != "")
                            {
                                createFile(tsFolder + "\\" + readCell(i + k, 4).ToString().Replace("\n", "") + ".tc");

                                testCaseFilePathList.Add(tsFolder + "\\" + readCell(i + k, 4).ToString().Replace("\n", "") + ".tc");
                                testCaseFilePath = testCaseFilePathList.ToArray();

                               
                                testCaseNOList.Add(readCell(i + k, 4).ToString().Replace("\n", ""));
                                testCaseNO = testCaseNOList.ToArray();

                                
                                testCaseNameList.Add(readCell(i + k, 5).ToString().Replace("\n", ""));
                                testCaseName = testCaseNameList.ToArray();

                                 
                                testCaseLevelList.Add(readCell(i + k, 6).ToString().Replace("\n", ""));
                                testCaseLevel = testCaseLevelList.ToArray();

                                
                                testCasepreConditionsList.Add(readCell(i + k, 7).ToString().Replace("\n", ""));
                                testCasepreConditions = testCasepreConditionsList.ToArray();

                                
                                testCaseStepsList.Add(readCell(i + k, 8).ToString().Replace("\n", ""));
                                testCaseSteps = testCaseStepsList.ToArray();

                                 
                                testCaseSampList.Add(readCell(i + k, 9).ToString().Replace("\n", ""));
                                testCaseSamp = testCaseSampList.ToArray();

                                
                                testCaseExpectList.Add(readCell(i + k, 10).ToString().Replace("\n", ""));
                                testCaseExpect = testCaseExpectList.ToArray();

                                
                                testCaseResultsList.Add(readCell(i + k, 11).ToString().Replace("\n", ""));
                                testCaseResults = testCaseResultsList.ToArray();

                                
                                testCaseIfAutoList.Add(readCell(i + k, 12).ToString().Replace("\n", ""));
                                testCaseIfAuto = testCaseIfAutoList.ToArray();

                                 
                                testCaseDefList.Add(Regex.Match(readCell(i + k, 4).ToString().Replace("\n", ""), "[]^[0-9]{3}$").Value);
                        ...
                        for (int n = 0; n < testCaseName.Length; n++)
                        {
                            writeTCFile(testCaseFilePath[n]);
                        }                       
                    }
                    else
                    {
            ...
            MessageBox.Show("项目生成成功！ 项目路径为：" + selectWorkspaceBrowserDialog.SelectedPath, "消息提示：", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void writeTSFile(string tsFilePath)
        {
            if (File.Exists(tsFilePath))
            {
                ...
                {
                    swts.WriteLine("                    测试用例{0}：{1}", i + 1, testCaseNO[i]);
                }
                swts.WriteLine("作 者：");
                swts.WriteLine("编写日期：{0}", DateTime.Now);
                swts.WriteLine("修改日期：");
                swts.WriteLine("");
                swts.WriteLine("'''");
                swts.WriteLine("");
                swts.WriteLine("");
                swts.WriteLine("import ssas.unittest,ssas.bs");
                swts.WriteLine("from ssas.logic import DataEncap,operate,expect");
                swts.WriteLine("");
                swts.WriteLine("");
                swts.WriteLine("class MyTestSuite(ssas.unittest.TestCase):");
                ...
                swts.WriteLine("        self.assertEqual(rs, True)");
                swts.WriteLine("");
                for (int i = 0; i < testCaseName.Length; i++)
                {
                    ...
                    swts.WriteLine("        self.assertEqual(rs, DataEncap(self.data_center[{0}]).expect.step1)");
                    swts.WriteLine("");
                    swts.WriteLine("");
                }
                ...
                swts.WriteLine("    ssas.unittest.main()");
                swts.Close();
            }
            else
            {
        ...
        private void writeTCFile(string tcFilePath)
        { 
        
            if (File.Exists(tcFilePath))
            {
                FileStream fstc = new FileStream(tcFilePath, FileMode.OpenOrCreate, FileAccess.ReadWrite);
                StreamWriter swtc = new StreamWriter(fstc);
                swtc.WriteLine("'''"); ;
                swtc.WriteLine("测试集编号：");
                swtc.WriteLine("                    {0}", testSuiteNO);
                swtc.WriteLine("测试用例编号：");
                swtc.WriteLine("                    {0}", testCaseNO[testCaseNO.Length-1]);
                swtc.WriteLine("作 者：");
                swtc.WriteLine("编写日期：{0}", DateTime.Now);
                swtc.WriteLine("修改日期：");
                swtc.WriteLine("");
                swtc.WriteLine("'''");
                swtc.WriteLine("");
                ...
                swtc.WriteLine("    ssas.unittest.main()");
                swtc.Close();
            }
            else
            {
